...
내 5개 작업 철학은 *무엇을 지킬지*는 완비돼 있지만 *어떻게 위반 불가능하게 만들지*는 인간 장인의 자기규율에 맡겨져 있다.
...
.claude/
  ...
  commands/scope.md      /scope  — 편집 경계 선언
  commands/horos-mode.md /horos-mode — 강도 토글
hooks/
...
bash tests/run.sh                           # 단위검증 (24 checks)
```
...
## 멀티머신 (M1 / Intel + Syncthing)

- 훅은 순수 bash + python3 stdlib 로만 동작한다 (venv·jq·절대경로 심링크 의존 없음).
- `.horos/` 는 gitignore 되며 머신 로컬이다. `decisions.jsonl` 은 공유 산출물이라 커밋 대상.
- `AGENTS.md` 가 정본이고 `CLAUDE.md`/`GEMINI.md` 는 그쪽으로의 *상대* 심링크다 — Intel(`msn-imac`)·M1(`msna-mba`) 양쪽에서 Syncthing 보존을 확인했다.

## 다음 단계

1. warn 으로 며칠 운용 → `horos log` 로 오탐 검토 → block 승격.
2. 철학 1(불신/계약): clamp·경계값 테스트 체크리스트를 스킬로. (기계 강제가 어려운 마지막 한 축)
3. 안정되면 fablize 구조처럼 `.claude-plugin/` 으로 추출해 전역·재사용 플러그인화.
